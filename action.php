...
if(isset($_POST["brand"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h4>Tiendas</h4></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$bid = $row["brand_id"];
			...
			echo "
				<div class='col-md-4'>
							<div class='panel panel-info'>
								...
								<div class='panel-heading'>$ $pro_price.00 MXN
		...
		if($count > 0){
			echo "
				<div class='alert alert-warning'>
						...
						<b>¡El producto ya está añadido en el carrito!</b>
				</div>
			";//not in video
		} else {
			...
			if (mysqli_query($con,$sql)) {
				echo "
					<div class='alert alert-success'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>¡Su producto se ha añadido con éxito!</b>
					</div>
				";
				exit();
			}
...
if(isset($_POST["getRecomienda"]) && isset($product_cat)){	
	...
	if(!isset($_SESSION['antecedente'])){
		echo " El antecedente es> ".$antecedente;
	...
	if (isset($_POST["checkOutDetails"])) {
		if (mysqli_num_rows($query) > 0) {
			...
			echo "<form method='post' action='login_form.php'>";
				...
				if (!isset($_SESSION["uid"])) {
					echo '<input type="submit" style="float:right;" name="login_user_with_product" class="btn btn-success btn-lg" value="Realizar Compra" >
							</form>';
					...
					echo '
						...
						<form action="https://www.sandbox.paypal.com" method="post">
							...
							echo   
								'<input type="hidden" name="return" value="http://localhost/ecome/payment_success.php"/>
					                <input type="hidden" name="notify_url" value="http://localhost/ecome/payment_success.php">
									<input type="hidden" name="cancel_return" value="http://localhost/ecome/cancel.php"/>
									<input type="hidden" name="currency_code" value="MXN"/>
									<input type="hidden" name="custom" value="'.$_SESSION["uid"].'"/>

									<input type="submit" style="float:right;" name="submit" class="btn btn-primary btn-lg " value="Pagar ahora con PayPal " >
...
if (isset($_POST["removeItemFromCart"])) {
	...
	if(mysqli_query($con,$sql)){
		echo "<div class='alert alert-danger'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>Producto Eliminado del carrito</b>
				</div>";
		exit();
	}
}
